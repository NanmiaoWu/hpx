...
namespace hpx {
    ...
    private:
        ...
        friend typename parallel::util::detail::algorithm_result<ExPolicy_>::type
        define_task_group(ExPolicy_&&, F&&);
    ...
    /// Constructs a \a task_group, \a tr, using the given execution policy
    ...
    typename parallel::util::detail::algorithm_result<ExPolicy>::type define_task_group(
        ExPolicy&& policy, F&& f)
    {
        ...
        task_group<policy_type> trh(std::forward<ExPolicy>(policy));
        ...
        {
            detail::handle_task_group_exceptions(trh.errors_);
        }
    ...
    /// Constructs a \a task_group, tr, and invokes the expression
    ...
    ///
    template <typename F>
    void define_task_group(F&& f)
    {
        define_task_group(hpx::execution::par, std::forward<F>(f));
    }
    ...
    /// \param policy       The execution policy to use for the scheduling of
    ///                     the iterations.
    /// \param f    The user defined function to invoke inside the define_task_group.
    ///             Given an lvalue \a tr of type \a task_group, the
    ///             expression, (void)f(tr), shall be well-formed.
    ///
    /// \throws An \a exception_list, as specified in Exception Handling.
    ///
    /// Postcondition: All tasks spawned from \a f have finished execution.
    ///                A call to \a define_task_group_restore_thread always
    ///                returns on the
    ...
    define_task_group_restore_thread(ExPolicy&& policy, F&& f)
    {
        static_assert(hpx::is_execution_policy<ExPolicy>::value,
            "hpx::is_execution_policy<ExPolicy>::value");

        // By design we always return on the same (HPX-) thread as we started
        // executing define_task_group_restore_thread.
        return define_task_group(
    ...
    ///
    template <typename F>
    void define_task_group_restore_thread(F&& f)
    {
        // By design we always return on the same (HPX-) thread as we started
        // executing define_task_group_restore_thread.
        define_task_group_restore_thread(
